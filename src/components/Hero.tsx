...
export default function Hero() {
  return (
    <section className="relative w-full h-[100svh] bg-[#E5F1FB] overflow-hidden">
      ...
      <div className="absolute inset-0 flex flex-col md:flex-row pointer-events-none z-20 pt-20 md:pt-0">
        
        {/* Left Column: The Model */}
        <div className="relative w-full md:w-1/2 h-full flex items-center justify-center overflow-visible pointer-events-auto z-30 pt-10 md:pt-0">
          
          {/* Animated Comic Card Container */}
          <motion.div 
            whileHover={{ scale: 1.02, rotate: -2, y: -10 }}
            className="relative w-[95%] md:w-[130%] lg:w-[650px] h-[300px] md:h-[450px] bg-white border-[6px] md:border-[8px] border-black rounded-[32px] shadow-[16px_16px_0_#111] overflow-hidden flex flex-row items-center group cursor-pointer transition-shadow hover:shadow-[24px_24px_0_#111] ml-0 md:ml-12"
          >
            ...
            <div className="relative w-1/2 h-full flex flex-col items-center justify-center z-20 pl-4 md:pl-8">
              <motion.div 
                className="font-cartoon text-5xl md:text-7xl text-[#FFD700] rotate-[-10deg] opacity-90 group-hover:scale-110 transition-transform duration-300 flex flex-col leading-[0.8]" 
                style={{ WebkitTextStroke: '2px #111', textShadow: '4px 4px 0 #111' }}
              >
                <span>STAY</span>
                <span className="text-[var(--color-coral-red)] text-6xl md:text-8xl ml-4">FRESH</span>
              </motion.div>
            </div>
            ...
            {/* Comic Sticker */}
            <div className="absolute top-4 left-4 md:top-6 md:left-6 bg-[var(--color-electric-blue)] border-[3px] border-black shadow-[4px_4px_0_#111] px-4 py-1 rotate-[-6deg] z-30 group-hover:rotate-[4deg] transition-transform">
              <span className="font-cartoon text-white text-base md:text-xl tracking-widest">VOL. 1</span>
            </div>
        ...
        <div className="relative w-full md:w-1/2 h-full flex flex-col items-center justify-center pointer-events-auto px-4">
          
          {/* Stacked Bubbly Typography */}
          <div className="flex flex-col font-cartoon text-[22vw] md:text-[16vw] leading-[0.8] text-white tracking-widest drop-shadow-[0_20px_20px_rgba(0,0,0,0.2)]" style={{ WebkitTextStroke: '6px #111', textShadow: '12px 12px 0 #111' }}>
            <div className="flex justify-center md:justify-start">
              <motion.span animate={{ y: [0, -20, 0], rotate: [0, -5, 0] }} transition={{ duration: 4, repeat: Infinity, delay: 0, ease: "easeInOut" }} className="inline-block">I</motion.span>
            </div>
            <div className="flex mt-4 md:mt-8">
              {["L", "I", "K", "E", "D"].map((letter, i) => (
                <motion.span key={i} animate={{ y: [0, -20, 0], rotate: [0, i % 2 === 0 ? 5 : -5, 0] }} transition={{ duration: 4, repeat: Infinity, delay: (i + 1) * 0.15, ease: "easeInOut" }} className="inline-block">{letter}</motion.span>
              ))}
            </div>
          </div>

          {/* Comic Speech Bubble CTA */}
          <a href="#shop" className="mt-8 md:mt-16 group cursor-pointer ml-0 md:ml-12">
            <motion.div whileHover={{ scale: 1.1, rotate: 5 }} whileTap={{ scale: 0.95 }} className="relative bg-white border-[5px] border-black rounded-[40px] px-8 py-4 md:px-10 md:py-6 shadow-[8px_8px_0_#111] transition-transform">
              ...
              <div className="flex flex-col items-center justify-center transform -rotate-2">
                <span className="font-cartoon text-black text-xl md:text-3xl tracking-widest leading-none">SHOP</span>
